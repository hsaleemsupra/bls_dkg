...
pub struct BivariatePolynomial {
    ...
    degree_x: usize,
    degree_y: usize,
    /// The coefficients of the polynomial. There are degree * degree terms in the polynomial
    coeff: Vec<Fr>
}
...
impl Debug for BivariatePolynomial {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.debug_struct("BivariatePolynomial")
            .field("BivariatePolynomial", &self.degree_x)
            .field("BivariatePolynomial", &self.degree_y)
            .field("BivariatePolynomial", &"...")
            .finish()
    }
}
...
#[allow(clippy::suspicious_op_assign_impl)]
impl<B: Borrow<BivariatePolynomial>> ops::AddAssign<B> for BivariatePolynomial {
    fn add_assign(&mut self, rhs: B) {
        let len = self.coeff.len();
        let rhs_len = rhs.borrow().coeff.len();
        if rhs_len > len {
...
impl<'a, B: Borrow<BivariatePolynomial>> ops::Add<B> for &'a BivariatePolynomial {
    type Output = BivariatePolynomial;

    fn add(self, rhs: B) -> BivariatePolynomial {
        (*self).clone() + rhs
    }
}

impl<B: Borrow<BivariatePolynomial>> ops::Add<B> for BivariatePolynomial {
    type Output = BivariatePolynomial;

    fn add(mut self, rhs: B) -> BivariatePolynomial {
        self += rhs;
        self
    }
}

impl<'a> ops::Add<Fr> for BivariatePolynomial {
    type Output = BivariatePolynomial;
...
/*impl<'a> ops::Add<u64> for BivariatePolynomial {
    type Output = BivariatePolynomial;
...
impl<B: Borrow<BivariatePolynomial>> ops::SubAssign<B> for BivariatePolynomial {
    fn sub_assign(&mut self, rhs: B) {
        let len = self.coeff.len();
        let rhs_len = rhs.borrow().coeff.len();
        if rhs_len > len {
...
impl<'a, B: Borrow<BivariatePolynomial>> ops::Sub<B> for &'a BivariatePolynomial {
    type Output = BivariatePolynomial;

    fn sub(self, rhs: B) -> BivariatePolynomial {
        (*self).clone() - rhs
    }
}

impl<B: Borrow<BivariatePolynomial>> ops::Sub<B> for BivariatePolynomial {
    type Output = BivariatePolynomial;

    fn sub(mut self, rhs: B) -> BivariatePolynomial {
        self -= rhs;
        self
    }
}

impl<'a> ops::Sub<Fr> for BivariatePolynomial {
    type Output = BivariatePolynomial;
...
/*impl<'a> ops::Sub<u64> for BivariateCommitment {
    type Output = BivariatePolynomial;
...
// Clippy thinks using any `+` and `-` in a `Mul` implementation is suspicious.
/*#[allow(clippy::suspicious_arithmetic_impl)]
impl<'a, B: Borrow<Poly>> ops::Mul<B> for &'a BivariatePolynomial {
    type Output = BivariatePolynomial;

    fn mul(self, rhs: B) -> Self::Output {
        let rhs = rhs.borrow();
        if rhs.is_zero() || self.is_zero() {
            return BivariatePolynomial::zero();
        }
        let n_coeffs = self.coeff.len() + rhs.coeff.len() - 1;
        let mut coeffs = vec![Fr::zero(); n_coeffs];
        let mut tmp = Fr::zero();
        ...
        clear_fr(&mut tmp);
        BivariatePolynomial::from(coeffs)
    }
}

impl<B: Borrow<BivariatePolynomial>> ops::Mul<B> for BivariatePolynomial {
    type Output = BivariatePolynomial;
...
impl<B: Borrow<Self>> ops::MulAssign<B> for BivariatePolynomial {
    fn mul_assign(&mut self, rhs: B) {
        *self = &*self * rhs;
    }
}

impl ops::MulAssign<Fr> for BivariatePolynomial {
    fn mul_assign(&mut self, rhs: Fr) {
        if bool::from(rhs.is_zero()) {
            self.zeroize();
            self.coeff.clear();
...
impl<'a> ops::Mul<&'a Fr> for BivariatePolynomial {
    type Output = BivariatePolynomial;
...
impl ops::Mul<Fr> for BivariatePolynomial {
    type Output = BivariatePolynomial;
...
impl<'a> ops::Mul<&'a Fr> for &'a BivariatePolynomial {
    type Output = BivariatePolynomial;
...
impl<'a> ops::Mul<Fr> for &'a BivariatePolynomial {
    type Output = BivariatePolynomial;
...
impl ops::Mul<u64> for BivariatePolynomial {
    type Output = BivariatePolynomial;
...
impl BivariatePolynomial {
    ...
    pub fn random<R: Rng>(degree_x: usize, degree_y: usize, rng: &mut R) -> Self {
        BivariatePolynomial::try_random(degree_x, degree_y, rng).unwrap_or_else(|e| {
            panic!(
                "Failed to create random `BivarPoly` of degree_x {}, degree_y {}: {}",
                degree_x, degree_y, e
            )
        })
    }
    ...
    /// Creates a random polynomial.
    pub fn try_random<R: Rng>(degree_x: usize, degree_y: usize, rng: &mut R) -> Result<Self> {

        let len = coeff_length(degree_x, degree_y).ok_or(Error::DegreeTooHigh)?;
    ...
    /// Returns the polynomial's x degree;
    pub fn degree_x(&self) -> usize {
        self.degree_x
    }
    /// Returns the polynomial's y degree;
    pub fn degree_y(&self) -> usize {
        self.degree_y
    }

    /// Returns index of the term in the polynomial where degree x = i and degree y = j
    pub fn coeff_pos(&self, i: usize, j: usize) -> Option<usize> {
        j.checked_add(i.checked_mul(self.degree_y.checked_add(1)?)?)
    }

    /// Returns the polynomial's value at the point `(x, y)`.
    pub fn evaluate<T: IntoFr>(&self, x: T, y: T) -> Fr {
        let x_pow = self.powers(x, self.degree_x);
        let y_pow = self.powers(y, self.degree_y);

        let mut result = Fr::zero();
        for (i, x_pow_i) in x_pow.into_iter().enumerate() {
            for (j, y_pow_j) in y_pow.iter().enumerate() {
                let index =  self.coeff_pos(i,j).expect("polynomial degree too high");
    ...
    /// Returns the f(x=k,y) as a univariate polynomial.
    pub fn get_univariate_poly<T: IntoFr>(&self, x: T) -> Poly {

        let mut result = self.row(0,x);

        for i in 1..self.degree_x+1{

            let row = self.row(i, x);
            result = result + row;
        }
    ...
    /// Returns the `row`-th row, as a univariate polynomial with x=k.
    pub fn row<T: IntoFr>(&self, row: usize, x: T) -> Poly {

        let mut coeff: Vec<Fr> = Vec::new();
        let x_pow_row = power(x, row);

        for j in 0..self.degree_y+1{

            let index = self.coeff_pos(row, j).expect("polynomial degree too high");
            let mut summand = self.coeff[index];
            summand.mul_assign(x_pow_row);
            coeff.push(summand);
    ...
    pub fn commitment(&self) -> BivariateCommitment {
    ...
    fn powers<T: IntoFr>(&self, x: T, degree: usize) -> Vec<Fr> {
        powers(x, degree)
    }
    ...
    pub fn reveal(&self) -> String {
        format!(
            "BivariatePolynomial {{ degree_x: {}, degree_y: {}, coeff: {:?} }}",
    ...
    /// Deserializes from big endian bytes
    pub fn from_bytes(bytes: Vec<u8>, degree_x: usize, degree_y: usize) -> Result<Self> {
        let mut c: Vec<Fr> = vec![];
        let coeff_size = bytes.len() / SK_SIZE;
        ...
        if (degree_x+1)*(degree_y+1)!=coeff_size {
            return Err(Error::DegreeTooHigh);
        }

        Ok(BivariatePolynomial {
            degree_x,
            degree_y,
            coeff: c,
        })
...
/// Our custom commitment to a symmetric bivariate polynomial.
...
impl Hash for BivariateCommitment {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.degree_x.hash(state);
        self.degree_y.hash(state);
        for c in &self.coeff {
            c.to_affine().to_compressed().as_ref().hash(state);
        }
    }
...
/*impl PartialOrd for BivariateCommitment {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for BivariateCommitment {
    fn cmp(&self, other: &Self) -> Ordering {
...
*/
impl<B: Borrow<BivariateCommitment>> ops::AddAssign<B> for BivariateCommitment {
    fn add_assign(&mut self, rhs: B) {
        let len = cmp::max(self.coeff.len(), rhs.borrow().coeff.len());
        self.coeff.resize(len, G1::identity());
        for (self_c, rhs_c) in self.coeff.iter_mut().zip(&rhs.borrow().coeff) {
...
impl<'a, B: Borrow<BivariateCommitment>> ops::Add<B> for &'a BivariateCommitment {
    type Output = BivariateCommitment;

    fn add(self, rhs: B) -> BivariateCommitment {
        (*self).clone() + rhs
    }
}

impl<B: Borrow<BivariateCommitment>> ops::Add<B> for BivariateCommitment {
    type Output = BivariateCommitment;

    fn add(mut self, rhs: B) -> BivariateCommitment {
        self += rhs;
        self
    }
}
...
impl BivariateCommitment {
    ...
    /// Returns the index of the term where x degree = i and y degree = j
    pub fn coeff_pos(&self, i: usize, j: usize) -> Option<usize> {
...
/// Returns the `i`-th power of `x`.
fn power<T: IntoFr>(into_x: T, i: usize) -> Fr {
    let x = into_x.into_fr();
    let mut x_pow_i = Fr::one();
...
/// Returns the total terms in a bivariate polynomial where degree_x is the degree of first variable
/// and degree_y is the degree of the second variable
pub fn coeff_length(degree_x: usize, degree_y: usize) -> Option<usize> {

    degree_x.checked_add(1)?.checked_mul(degree_y.checked_add(1)?)
}
